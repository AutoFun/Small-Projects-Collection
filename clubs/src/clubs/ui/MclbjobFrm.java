...
public class MclbjobFrm implements ActionListener {	
	String cksql="select clbjob.* from clbjob where 1=1 ";
	JFrame w;
	JDialog jdl=new JDialog(w,"社团职务管理",true);
	//显示信息的表  id 学号
	String[] listcols={"id","职务名称"};
	//设置表格整行显示
	...
	//查询模块
	JLabel ckclasslabel = new JLabel("职位名称：");
	JTextField ckclasstext=new JTextField();
	JButton checkbt = new JButton("查询");
	//查询条件
	JLabel no=new JLabel("");
	JLabel no2=new JLabel("");
	//管理模块 学号
	JLabel mgjbnamelb=new JLabel("职务名称：");
	JTextField mgjbnametxt=new JTextField();
	...
	MclbjobFrm(JFrame jf){
		...
		//数据管理部分布局
		jpl.add(mgjbnamelb);
		mgjbnamelb.setBounds(20, 20, 70, 20);
		jpl.add(mgjbnametxt);
		mgjbnametxt.setBounds(90, 20, 100, 20);
		...
		slist.addMouseListener(new java.awt.event.MouseAdapter(){
            public void mouseClicked(MouseEvent e) {
              ...
              mgjbnametxt.setText(slist.getValueAt(r, 1).toString());
            }
        });
	...
	class CheckAction implements ActionListener{		
		...
		@Override
		public void actionPerformed(ActionEvent e) {
			no2.setText(ckclasstext.getText());
	...
	class AddAction implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			// TODO Auto-generated method coub
			String jbname=mgjbnametxt.getText().trim();
			//信息完整性验证
			if("".equals(jbname)){
				JOptionPane.showMessageDialog(null,"职务信息不完整!", "系统信息", JOptionPane.WARNING_MESSAGE);	
				return;
			}
			//创建数据库操作对象
			DbConn db=new DbConn();
			//职务唯一性验证
			if(db.checkTrue("select id from clbjob where jbname='"+jbname+"'")){
				JOptionPane.showMessageDialog(null,"职务已存在!", "系统信息", JOptionPane.WARNING_MESSAGE);	
				return;
			}
			//添加信息
			if(db.executeUpdate("insert into clbjob(jbname) values('"+jbname+"')") >0){
	...
	class DelAction implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			DbConn db=new DbConn();
			if(db.executeUpdate("delete from clbjob where id="+id)>0){
	...
	class UpdAction implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			DbConn db=new DbConn();
			//学号唯一性验证
			if(db.checkTrue("select id from clbjob where jbname='"+jbname+"' and id!="+id)){
				JOptionPane.showMessageDialog(null,"职务已存在!", "系统信息", JOptionPane.WARNING_MESSAGE);	
				return;
			}
			if(db.executeUpdate("update clbjob set jbname='"+jbname+"' where id="+id)>0){
	...
	public void getData(String cksql){
		...
		if(!"".equals(no2.getText())){
			cksql+=" and jbname like '%"+no2.getText().trim()+"%'";
		}
		DbConn db=new DbConn();
		...
		if(rs!=null){
			try {
				while(rs.next()){
					//{"id","学号"};
					model.addRow(new String[]{String.valueOf(rs.getInt("ID")),rs.getString("jbname")});
				}
